...
public class GateTeleport : MonoBehaviour
{
    ...
    [SerializeField] private GameObject vfxMago;
 
    [Header("Tiempos")]
    [SerializeField] private float delayAntesDeViajar = 1f; // Pausa dramática antes de iniciar
    [SerializeField] private float cooldownFinal = 1f;       // Seguridad al llegar

    [Header("Curva del Viaje")]
    [SerializeField] private float amplitud = 1.5f;
    ...
    private IEnumerator DoTeleport(GameObject playerGO)
    {
        ...
        {
            yield return StartCoroutine(MoverHacia(
                playerGO.transform, waypoint.position,
                camTransform, camOffset, camSmooth
            ));
        }
 
        // Último tramo: viajamos al spawnPoint del portal destino
        yield return StartCoroutine(MoverHacia(
            playerGO.transform, spawnPoint.position,
            camTransform, camOffset, camSmooth
        ));
   ...
   private IEnumerator MoverHacia(Transform objeto, Vector3 destino, Transform camTransform, Vector3 camOffset, float camSmooth)
    {
        ...
        Vector3 desplazamiento = new Vector3(Random.Range(-1f, 1f),0f,Random.Range(-1f, 1f)).normalized * amplitud;
