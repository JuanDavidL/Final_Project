...
public class ProcessingMachineLogic : MonoBehaviour
{
    ...
    [SerializeField]
    private ItemData potionBasura;

    [SerializeField]
    private GameObject potionBad;
    private ItemData _lastResult;

    [Header("Ingredientes en el Contenedor")]
    private List<InventoryManager.InventorySlot> currentIngredients =
        new List<InventoryManager.InventorySlot>();

    [Header("Referencias de Botones")]
    [SerializeField]
    private MachineButton btnRojo;

    [SerializeField]
    private MachineButton btnAmarillo;

    [SerializeField]
    private MachineButton btnVerde;

    [Header("Animaciones/Objetos")]
    [SerializeField]
    private GameObject contenedorFisico;
    ...
    private void TryOpenMachine()
    {
        ...
        if (_waitingForYellow)
            return;

        // ← bloquea si está procesando o lista
        if (currentState == MachineState.Procesando || currentState == MachineState.Lista)
            return;
    ...
    private void TryDeliverPotion()
    {
        if (currentState == MachineState.Lista && _lastResult != null)
        {
            ...
            GameObject prefabAFabricar =
                (_lastResult == potionBasura) ? potionBad : selectedRecipe.potionPrefab;
    ...
    private IEnumerator ProcesoEnvioInventario(PoolableItem itemVisual, GameObject prefabUsado)
    {
        ...
        while (timer < 2.0f)
        {
            itemVisual.transform.position =
                startPos + new Vector3(0, Mathf.Sin(Time.time * 5f) * 0.1f, 0);
    ...
    {
        if (currentState != MachineState.Recibiendo)
            return;

        // Verifica si este ingrediente pertenece a la receta
        bool isCorrect = selectedRecipe.requiredIngredients.Exists(i => i.item == data);
        ...
        if (existing != null)
            ...
            currentIngredients.Add(
                new InventoryManager.InventorySlot { item = data, quantity = 1 }
            );
    ...
    private bool ValidarIngredientes()
    {
        if (selectedRecipe == null)
            return false;
        if (currentIngredients.Count != selectedRecipe.requiredIngredients.Count)
            return false;
    ...
    {
        if (selectedRecipe == null || currentState != MachineState.Procesando)
            return;
    ...
    private IEnumerator MoverContenedor(float targetZ)
    {
        ...
        {
            contenedorFisico.transform.localPosition = Vector3.Lerp(
                startPos,
                endPos,
                elapsed / duration
            );
